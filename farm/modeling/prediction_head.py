...
class TextSimilarityHead(PredictionHead):
    ...
    def __init__(self, similarity_function: str = "dot_product", global_loss_buffer_size: int = 150000, **kwargs):
        ...
        self.generate_config()
        # self.dense1 = nn.Linear(768, 256, bias=False)
        # self.dense2 = nn.Linear(768, 256, bias=False)
    ...
    def forward(self, query_vectors:torch.Tensor, passage_vectors:torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        return (query_vectors, passage_vectors) # (self.dense1(query_vectors), self.dense2(passage_vectors))
    ...
    def logits_to_loss(self, logits: Tuple[torch.Tensor, torch.Tensor], **kwargs):
        ...
        # Calculate loss
        loss = self.loss_fct(softmax_scores, targets) # + torch.norm(torch.eye(768, device=softmax_scores.device)-torch.mm(self.dense1.weight.t(), self.dense2.weight))
        return loss
